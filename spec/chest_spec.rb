...
describe Chest do
  ...
  it 'can be summoned with items already in it' do
    ...
    expect(SummonCommand.new(chest).to_s).to eq %q<summon chest {Items:[{Slot:0,id:diamond_sword,Count:64},{Slot:3,id:bow,Count:63}],display:{Name:"Chest of Wonders"}}>
  end
  ...
  it 'can be given with items already in it' do
    ...
    expect(ItemCommand.new(item).to_s).to eq %q<i chest 1 {BlockEntityTag:{Items:[{Slot:0,id:diamond_sword,Count:2},{Slot:3,id:bow,Count:3}]},display:{Name:"Chest of Wonders!"}}>
  end
end
